...
def get_suggestions(product_name, style, colour_theme):
    ...
    if not matched:
        matched = {
            "colours":    f"Research competitor palettes for {product_name}",
            "typography": "Bold clear font for brand name",
            "layout":     "Brand name top, product hero centre, key claims bottom",
            "imagery":    f"High-quality {product_name} photography",
            "mood":       "Professional, trustworthy",
            "tips":       [
                f"Research top 3 competitor {product_name} brands",
                "Hero product photograph fills at least 35% of front face",
                "Brand name readable from 3 metres away",
                "Highlight #1 unique selling point prominently",
                "Consistent visual language across all variants",
            ],
        }
    result = dict(matched)
    result["style_colours"] = {
        "Bold & Vibrant": "High saturation reds, oranges, yellows",
        "Eco-Friendly":   "Earthy greens, kraft browns",
        "Minimalist":     "Clean whites, soft greys",
        "Interactive":    "Dynamic gradients, AR zones",
    }.get(style, "")
    result["colour_theme"] = {
        "🔥 Warm":  "Reds, oranges, golds",
        "❄️ Cool":   "Blues, teals, silvers",
        "🌿 Earth": "Greens, browns, creams",
        "🎨 Bold":  "Vivid multicolour",
        "🖤 Dark":  "Black, charcoal, navy",
    }.get(colour_theme, "")
    return result
    ...
    if uploaded:
        ...
        with col_res:
            ...
            st.markdown(
                ...
                f'<div class="strategy-badge" style="background:{grad};color:white">'
                f'{report["strategy"]["code"]} — {sname}</div>'
                f'<p style="color:rgba(255,255,255,0.6);font-size:0.9rem;margin-top:8px">'
                f'{report["strategy"]["description"]}</p></div>',
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            f'<div style="background:rgba(102,126,234,0.15);border-radius:12px;padding:16px;margin-top:12px;text-align:center">'
            f'New CS: <strong style="color:#667eea;font-size:1.4rem">{new_cs:.3f}</strong> &nbsp;|&nbsp; '
            f'Change: <strong style="color:{dcol}">{dsign}{delta:.3f}</strong> &nbsp;|&nbsp; '
            f'Strategy: <strong style="color:{dcol}">{new_st["name"]}</strong></div>',
            unsafe_allow_html=True,
        )
        st.markdown("</div>", unsafe_allow_html=True)
...
with tab2:
    ...
    if up_a and up_b:
        ...
        st.markdown(
            f'<div class="winner-banner" style="background:{wg}">🏆 Design {winner} Wins!'
            f'<div style="font-size:1rem;font-weight:400;margin-top:6px;opacity:0.9">'
            f'CS_A = {comp["design_a_cs"]} &nbsp;vs&nbsp; CS_B = {comp["design_b_cs"]}</div></div>',
            unsafe_allow_html=True,
        )
...
with tab3:
    ...
    gg, gc = {
        "🎯 Maximum Customer Attraction": ("linear-gradient(135deg,#ff416c,#ff4b2b)", "#ff416c"),
        "🌿 Most Eco-Friendly":           ("linear-gradient(135deg,#11998e,#38ef7d)", "#11998e"),
        "🧠 Strongest Brand Memory":      ("linear-gradient(135deg,#4776e6,#8e54e9)", "#4776e6"),
        "⚖️ Best Overall Balance":        ("linear-gradient(135deg,#667eea,#764ba2)", "#667eea"),
        "🏪 Best Shelf Visibility":       ("linear-gradient(135deg,#f7971e,#ffd200)", "#f7971e"),
    }.get(goal, ("linear-gradient(135deg,#667eea,#764ba2)", "#667eea"))

    st.markdown(
        f'<div style="background:{gg};border-radius:12px;padding:14px 20px;margin:8px 0">'
        f'<strong style="color:white">{goal}</strong><br>'
        f'<span style="color:rgba(255,255,255,0.85);font-size:0.9rem">{GOAL_DESC[goal]}</span></div>',
        unsafe_allow_html=True,
    )
    ...
    for i, col in enumerate(cols_up):
        ...
        if f:
            up_designs.append((design_labels[i], f))
    ...
    if len(up_designs) >= 2:
        if st.button("🚀 Find Best Design for My Goal"):
            ...
            with st.spinner("Analyzing all designs..."):
                for name, file in up_designs:
                    ...
                    results.append({
                        ...
                        "image":      img,
                        "scores":     scores,
                        "goal_score": compute_goal_score(scores, goal),
                        "cs":         cs,
                        "strategy":   strat,
                    })
            ...
            st.markdown(
                f'<div class="winner-banner" style="background:{wg}">🏆 RECOMMENDED: {winner["name"]}'
                f'<div style="font-size:1rem;font-weight:400;margin-top:8px;opacity:0.9">'
                f'Best for: {goal} &nbsp;|&nbsp; Score: {winner["goal_score"]:.3f} &nbsp;|&nbsp; {winner["strategy"]["name"]}</div></div>',
                unsafe_allow_html=True,
            )

            ri   = ["🥇","🥈","🥉","4️⃣","5️⃣"]
            ic   = st.columns(len(results))
            for i, (col, r) in enumerate(zip(ic, results)):
                ...
                col.markdown(
                    ...
                    f'<div style="font-size:1.5rem;font-weight:700;color:{rc}">{r["goal_score"]:.3f}</div>'
                    f'<div style="color:rgba(255,255,255,0.6);font-size:0.8rem">Goal Score</div>'
                    f'<div style="color:{rc};font-size:0.85rem;font-weight:600">{r["strategy"]["name"]}</div></div>',
                    unsafe_allow_html=True,
                )

            ws  = winner["scores"]
            bc  = max(CRITERIA, key=lambda c: ws[c])
            wkc = min(CRITERIA, key=lambda c: ws[c])
            st.markdown(
                f'<div class="recommend-card"><h4>💡 Why {winner["name"]} is Recommended</h4>'
                f'<div class="tip-card">✅ <strong>Strongest:</strong> {LABELS[bc]} ({ws[bc]:.2f})</div>'
                f'<div class="tip-card">✅ <strong>Strategy:</strong> {winner["strategy"]["name"]} — {winner["strategy"]["description"]}</div>'
                f'<div class="tip-card">⚠️ <strong>Improve:</strong> {LABELS[wkc]} ({ws[wkc]:.2f})</div></div>',
                unsafe_allow_html=True,
            )

    elif len(up_designs) == 1:
        st.info("⚠️ Upload at least 2 designs to compare")
    else:
        st.markdown('<div style="text-align:center;padding:40px;color:rgba(255,255,255,0.4)"><div style="font-size:3rem">📦</div><div>Upload 2–5 designs above to get started</div></div>', unsafe_allow_html=True)
...
with tab4:
    ...
    if st.button("✨ Generate Design Suggestions + Sample Image"):
        if not pname:
            ...
            with cs1:
                st.markdown(
                    f'<div class="gen-card"><h4>🎯 Design Brief: {pname}</h4>'
                    f'<p style="color:rgba(255,255,255,0.6);font-size:0.85rem">{dstyle} · {ctheme} · {taud} · {mseg}</p></div>',
                    unsafe_allow_html=True,
                )
                for title, key in [
                    ("🎨 Recommended Colours", "colours"),
                ...
                for i, tip in enumerate(sug["tips"], 1):
                    st.markdown(f'<div class="suggestion-card"><strong>#{i}</strong> {tip}</div>', unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
                ...
                with st.spinner("🎨 Generating... (10-20 seconds)"):
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            ib = BytesIO()
                            gi.save(ib, format="PNG")
                            st.download_button(
                                "⬇️ Download Image",
                                data=ib.getvalue(),
                                file_name=f"{pname.replace(' ','_')}_concept.png",
                                mime="image/png",
                            )
                        else:
                            st.warning("Image service timed out. Try again.")
                    except Exception:
                        st.warning("Image service busy. Try again in a moment.")
                st.markdown("</div>", unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════════════════
# TAB 5 — ELECTRONICS AI (single-item analyser)
# ══════════════════════════════════════════════════════════════════════════════
with tab5:
    st.markdown("""
    <div class="elec-card">
    ...
    if elec_model is None:
        ...
        with eb:
            ...
            if up_ei:
                ...
                pil_ei = None
                st.markdown(
                    '<div style="background:rgba(255,255,255,0.03);border:2px dashed rgba(56,239,125,0.3);'
                    'border-radius:12px;padding:40px;text-align:center;color:rgba(255,255,255,0.4)">'
                    '<div style="font-size:2.5rem">📷</div>'
                    '<div style="margin-top:8px;font-size:0.85rem">Image optional — text alone works fine</div></div>',
                    unsafe_allow_html=True,
                )
        ...
        if st.button("🔬 Predict Brand Attention", key="elec_btn"):
            if not ep_rev.strip():
                ...
                with st.spinner("🤖 Analysing with Multimodal AI..."):
                    pred, probs = predict_electronics(
                        f"{ep_name} {ep_rev}".strip(), pil_ei,
                        elec_model, elec_device, elec_tok,
                    )

                label, color, desc = ELEC_LABELS[pred]
                st.markdown(
                    f'<div style="background:linear-gradient(135deg,{color}33,{color}11);'
                    f'border:2px solid {color};border-radius:16px;padding:24px;text-align:center;margin:16px 0">'
                    f'<div style="font-size:2rem;font-weight:700;color:{color}">{label}</div>'
                    f'<div style="color:rgba(255,255,255,0.7);margin-top:8px">{desc}</div></div>',
                    unsafe_allow_html=True,
                )
                ...
                for i, (cn, cc) in enumerate([("⚠️ Negative","#ff416c"),("😐 Neutral","#ffd200"),("🌟 Positive","#38ef7d")]):
                    ...
                    st.markdown(
                        ...
                        f'<div style="display:flex;justify-content:space-between;color:rgba(255,255,255,0.7);font-size:0.85rem;margin-bottom:4px">'
                        f'<span>{cn}</span><span style="color:{cc};font-weight:700">{pct}%</span></div>'
                        f'<div style="background:rgba(255,255,255,0.1);border-radius:50px;height:10px">'
                        f'<div style="width:{pct}%;background:{cc};height:10px;border-radius:50px;box-shadow:0 0 8px {cc}88"></div>'
                        f'</div></div>',
                        unsafe_allow_html=True,
                    )
                st.markdown("</div>", unsafe_allow_html=True)
                ...
                st.download_button(
                    ...
                    data=json.dumps({
                        ...
                        "recommendations": ELEC_ADVICE[pred],
                    }, indent=2),
                    file_name="electronics_analysis.json",
                    mime="application/json",
                )
...
with tab6:
    ...
    if not catalog:
        ...
        with col_f:
            top_n = st.selectbox("Show top", [3, 5, 10], key="top_n")
        ...
        if st.button("🚀 Find Best Products", key="find_btn"):
            if not query.strip():
                ...
                keyword_map = {
                    ...
                    "phone":"smartphone","smartphone":"smartphone","mobile":"smartphone","iphone":"smartphone","android":"smartphone",
                ...
                matched_cat = next((cat for kw, cat in keyword_map.items() if kw in q), None)

                if matched_cat:
                    filtered = [p for p in catalog if p["category"] == matched_cat]
                else:
                    filtered = [p for p in catalog if q in p["name"].lower() or q in p["brand"].lower() or q in p["category"].lower()]
                ...
                if not filtered:
                    ...
                    if model_loaded:
                        ...
                        for i, product in enumerate(filtered):
                            pos, neu, neg = get_attention_score(product["review"], model, tokenizer, device)
                            scored.append({
                                **product,
                            ...
                            prog.progress((i + 1) / len(filtered), text=f"Analysed: {product['name']}")
                                ...
                                <div style="background:rgba(255,255,255,0.04);
                                  ...
                                  <div style="display:flex;justify-content:space-between;align-items:center">
                                    <div>
                                      ...
                                      <strong style="font-size:1.2rem;color:white;margin-left:8px">{product["name"]}</strong>
                                      <span style="color:rgba(255,255,255,0.5);font-size:0.85rem;margin-left:8px">by {product["brand"]}</span>
                                    </div>
                                    <div style="text-align:right">
                                      <div style="font-size:2rem;font-weight:700;color:{badge_col}">{score:.0f}%</div>
                                      <div style="color:rgba(255,255,255,0.5);font-size:0.75rem">Brand Attention</div>
                                    </div>
                                  </div>
                                  <div style="margin-top:8px">
                                    <span style="background:{badge_col}33;color:{badge_col};padding:4px 12px;border-radius:20px;font-size:0.8rem;font-weight:600">{badge_txt}</span>
                                    <span style="color:rgba(255,255,255,0.5);font-size:0.85rem;margin-left:12px">💰 {product["price"]}</span>
                                  </div>
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                with st.expander(f"📊 See detailed scores for {product['name']}"):
                                    ...
                                    for col, lbl, val, clr in [
                                        ...
                                        <div style="text-align:center;background:rgba(255,255,255,0.05);border-radius:12px;padding:12px">
                                          <div style="color:{clr};font-size:1.5rem;font-weight:700">{val:.1f}%</div>
                                          <div style="color:rgba(255,255,255,0.6);font-size:0.8rem">{lbl}</div>
                                        </div>
                                        """, unsafe_allow_html=True)
...
with tab7:
    st.markdown(
        '<div class="card"><h3>🎯 About PackageAI</h3>'
        '<p style="color:rgba(255,255,255,0.7)">Packaging Attention Prediction + Electronics Brand Attention. '
        'Final Year Major Project 2024-25. Prasad V. Potluri Siddhartha Institute of Technology.</p></div>',
        unsafe_allow_html=True,
    )

    c1, c2, c3, c4, c5 = st.columns(5)
    for col, icon, title, desc in [
        (c1, "🧠", "MobileNetV2",  "Packaging Model"),
        (c2, "📊", "MACROS",       "Multi-Criteria Ranking"),
        (c3, "📦", "5058 Images",  "Biscuit Dataset"),
        (c4, "🔬", "BERT+CNN",     "Electronics Model"),
        (c5, "🎨", "Pollinations", "AI Image Gen"),
    ]:
        col.markdown(
            f'<div class="metric-box"><div style="font-size:2rem">{icon}</div>'
            f'<div style="font-weight:700;margin:8px 0">{title}</div>'
            f'<div style="color:rgba(255,255,255,0.6);font-size:0.85rem">{desc}</div></div>',
            unsafe_allow_html=True,
        )

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        '<div class="card"><h4>⚖️ MACROS Weights</h4>'
